...
void compress(std::string fname, size_t len1, size_t len2, size_t len3) {
  ...
  // lorenzo_optimizer(Grid1D, Block1D, data_len);

  // histogram kernel optimization
  histogram_optimizer(data_len, 1024, hist_grid_d, hist_block_d, hist_shmem_use,
                      hist_repeat);
  ...
  stf_internal_buffers ibuffer(ctx, data_len, input_data_host,
  ...
  ctx.task(ibuffer.l_uncomp.rw(), ibuffer.l_q_codes.write(),
           ibuffer.l_hist.write(), ibuffer.l_out_vals.write(),
           ibuffer.l_out_idxs.write(), ibuffer.l_out_num.write(),
           ibuffer.l_top1.write())
          .set_symbol("lorenzo_1d")
          ->*[&](cudaStream_t s, auto l_u, auto q_c, auto l_h, auto o_v,
                 auto o_i, auto o_n, auto l_t1) {
                // kernel_proto_lorenzo_1d<<<Grid1D, Block1D, 0, s>>>(l_u,
                // data_len, q_c, o_v, o_i, o_n, eb*2, 1/(eb*2));
                lorenzo_1d(l_u, data_len, q_c, o_v, o_i, o_n, l_t1, eb * 2,
                           1 / (eb * 2), 512, s);
              };

  //! Generic 2013 Histogram Kernel
  ctx.task(ibuffer.l_q_codes.read(), ibuffer.l_hist.write())
  ...
  ctx.task(ibuffer.hf_header_entry.write(), ibuffer.out_entries.write())
          .set_symbol("header_setup")
          ->*[&](cudaStream_t s, auto hf_h_entry, auto entry) {
                cuda_safe_call(cudaMemsetAsync(hf_h_entry.data_handle(), 0,
                                               6 * sizeof(uint32_t), s));
                cuda_safe_call(cudaMemsetAsync(entry.data_handle(), 0,
                                               5 * sizeof(uint32_t), s));
              };

  // fill in the entry array for data segments using prefix sum
  ctx.host_launch(ibuffer.out_entries.write(), ibuffer.l_out_num.read())
          .set_symbol("h_entries_fill")
          ->*
      [&](auto entry, auto out_num) {
  ...
  ctx.task(ibuffer.l_compressed.rw(), ibuffer.l_out_vals.read(), ibuffer.l_out_idxs.read())
    .set_symbol("concat_bitstream")->*[&](cudaStream_t s, auto compressed, auto out_vals, auto out_idxs) {
      ...
      cuda_safe_call(cudaMemcpyAsync(
          compressed.data_handle() + h_entries[SPFMT] + (num_outliers * sizeof(float)),
  ...
  ctx.task(ibuffer.l_compressed.read()).set_symbol("gpu_comp_to_file")->*[&](cudaStream_t s, auto compressed) {
  ...
  //! Output Data To File
  ctx.host_launch().set_symbol("file_output")->*[&]() {
      auto compressed_fname = fname + ".stf_compressed";

      // allocate space for compressed data on host of size compressed_len
      // auto file = MAKE_UNIQUE_HOST(uint8_t, compressed_len);

      // print host entries
      printf("Host Entries: [0]=%u [1]=%u [2]=%u [3]=%u [4]=%u [5]=%u\n",
              h_entries[0], h_entries[1], h_entries[2], h_entries[3],
              h_entries[4], h_entries[5]);
      ...
      auto writeField = [&ptr, &offset](const void* data, size_t dataSize) {
...
void decompress(std::string fname, size_t len1, size_t len2, size_t len3) {
  ...
  cudaMallocHost(&compressed_data_host, compressed_len * sizeof(uint8_t));
  utils::fromfile(compressed_fname.c_str(), compressed_data_host, compressed_len);
  ...
  std::memcpy(encoded_data_h, compressed_data_host + entries_file[ENCODED], encoded_size);
  ...
  ctx.task(ibuffer.l_uncompressed.write(), l_encoded.write(), l_spval.write(), l_spidx.write()).set_symbol("populate_outliers")
      ->*[&](cudaStream_t s, auto uncompressed, auto encoded, auto spval, auto spidx) {

    cuda_safe_call(cudaMemsetAsync(uncompressed.data_handle(), 0, data_len * sizeof(float), s));
    cuda_safe_call(cudaMemcpyAsync(encoded.data_handle(), encoded_data_h, encoded_size, cudaMemcpyHostToDevice, s));
    
    if (splen > 0) {
      cuda_safe_call(cudaMemcpyAsync(spval.data_handle(), outlier_vals_h, splen * sizeof(float), cudaMemcpyHostToDevice, s));
      cuda_safe_call(cudaMemcpyAsync(spidx.data_handle(), outlier_idx_h, splen * sizeof(uint32_t), cudaMemcpyHostToDevice, s));
    }
  };

  ctx.task(l_encoded.read()).set_symbol("hf_header_populate")->*[&](cudaStream_t s, auto encoded) {
    cuda_safe_call(cudaMemcpyAsync(&header_hf, encoded.data_handle(), sizeof(hf_header), cudaMemcpyDeviceToHost, s));
  };

  cuda_safe_call(cudaStreamSynchronize(ctx.task_fence()));

  ctx.task(l_spval.read(), l_spidx.read(), ibuffer.l_uncompressed.rw()).set_symbol("spv_scatter")
      ->*[&](cudaStream_t s, auto val, auto idx, auto uncompressed) {
    if (splen > 0) {
      spvn_scatter<<<(splen-1)/128+1, 128, 0, s>>>(val, idx, splen, uncompressed);
    }
  };

  ctx.task(l_encoded.rw(), ibuffer.l_q_codes.write()).set_symbol("hf_decode")->*[&](cudaStream_t s, auto encoded, auto q_codes) {
    auto div = [](auto l, auto subl) { return (l - 1) / subl + 1; };
    auto const block_dim = 256;
    auto const grid_dim = div(pardeg, block_dim);

    #define ACCESSOR(SYM, TYPE) reinterpret_cast<TYPE*>(encoded.data_handle() + header_hf.entry[HF_HEADER_##SYM])

    hf_kernel_decode<<<grid_dim, block_dim, codec_hf.revbk4_bytes, s>>>(ACCESSOR(BITSTREAM, uint32_t), ACCESSOR(REVBK, uint8_t), ACCESSOR(PAR_NBIT, uint32_t), ACCESSOR(PAR_ENTRY, uint32_t), codec_hf.revbk4_bytes, sublen, pardeg, q_codes);
  };

  // cuda_safe_call(cudaStreamSynchronize(ctx.task_fence()));

  ctx.task(ibuffer.l_q_codes.rw(), ibuffer.l_uncompressed.rw()).set_symbol("lorenzo_decomp")->*[&](cudaStream_t s, auto q_codes, auto uncompressed) {
    lorenzo_decomp_1d(q_codes, uncompressed, data_len, eb*2, 1/(eb*2), 512, s);
  };

  ctx.host_launch(ibuffer.l_uncompressed.read()).set_symbol("decomp_to_file")->*[&](auto uncompressed) {
...
void compare(std::string fname, size_t len1, size_t len2, size_t len3) {
  ...
  cudaMemcpy(original_data_device, original_data_host, len1 * len2 * len3 * sizeof(float), cudaMemcpyHostToDevice);
  cudaMemcpy(decompressed_data_device, decompressed_data_host, len1 * len2 * len3 * sizeof(float), cudaMemcpyHostToDevice);
  ...
  fz::module::GPU_extrema(original_data_device, len1 * len2 * len3, orig_data_res);
  fz::module::GPU_extrema(decompressed_data_device, len1 * len2 * len3, decomp_data_res);

  float h_err[4];

  fz::module::GPU_calculate_errors<float>(
      original_data_device, orig_data_res[AVGVAL], decompressed_data_device, decomp_data_res[AVGVAL], len1 * len2 * len3, h_err);
  ...
  fz::module::GPU_find_max_error<float>(
      decompressed_data_device, original_data_device, len1 * len2 * len3, max_abserr, max_abserr_index);
  ...
  printf("Normalized Root Mean Squared Error: %f\n", sqrt(mse) / (orig_data_res[MAXVAL] - orig_data_res[MINVAL]));
  printf("Peak Signal-to-Noise Ratio: %f\n", 20 * log10(orig_data_res[MAXVAL] - orig_data_res[MINVAL]) - 10 * log10(mse));
