...
struct stf_internal_buffers {
  stf_internal_buffers(context& ctx, size_t len, float* input_data, size_t revbk4_bytes, size_t bk_bytes, int pardeg, bool comp = true) {
    if (comp) {
      ...
      l_out_vals =  ctx.logical_data(shape_of<slice<float>>(len / 5)).set_symbol("l_out_vals");
      ...
      l_uncompressed = ctx.logical_data(shape_of<slice<float>>(len)).set_symbol("l_uncompressed");
      l_q_codes =   ctx.logical_data(shape_of<slice<uint16_t>>(len)).set_symbol("l_q_codes");
    }
  }
  mutable logical_data<slice<float>> l_uncomp;       // logical uncompressed data
  ...
  mutable logical_data<slice<float>> l_out_vals;     // outlier values
  ...
  mutable logical_data<slice<float>> l_uncompressed; // logical compressed data
};