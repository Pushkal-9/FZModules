...
__global__ void kernel_hist_generic(slice<const uint16_t> quant_codes,
  ...
  if (warp_id >= warps_block) end = data_len;

  for (unsigned int pos = threadIdx.x; pos < hist_len + 1; pos += blockDim.x) {
    Hs[pos] = 0;
  }
  __syncthreads();
  ...
  for (unsigned int pos = threadIdx.x; pos < hist_len; pos += blockDim.x) {
    int sum = 0;
    for (int base = 0; base < (hist_len + 1) * r_per_block;
         base += (hist_len + 1)) {
      sum += Hs[base + pos];
    }
