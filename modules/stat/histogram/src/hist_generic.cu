...
template <typename T>
__global__ void KERNEL_CUHIP_histogram_naive(T* in_data, size_t const data_len,
                                             uint32_t* out_bins,
...
__global__ void KERNEL_CUHIP_p2013Histogram(T* in_data, size_t const data_len,
                                            FREQ* out_bins,
                                            uint16_t const bins_len,
                                            uint16_t const repeat) {
...
void GPU_histogram_generic_optimizer_on_initialization(
    size_t const data_len, uint16_t const hist_len, int& grid_dim,
    int& block_dim, int& shmem_use, int& r_per_block) {
...
template <typename T>
int GPU_histogram_generic(T* in_data, size_t const data_len, uint32_t* out_hist,
                          uint16_t const hist_len, int const grid_dim,
                          int const block_dim, int const shmem_use,
                          int const r_per_block, void* stream) {
  KERNEL_CUHIP_p2013Histogram<<<grid_dim, block_dim, shmem_use,
                                (cudaStream_t)stream>>>  //
...
INIT_HIST_CUDA(uint8_t);
INIT_HIST_CUDA(uint16_t);
INIT_HIST_CUDA(uint32_t);
// INIT_HIST_CUDA(f4);
