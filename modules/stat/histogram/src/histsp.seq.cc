...
namespace fz::module {
  ...
  template <typename E>
  int SEQ_histogram_Cauchy_v1(E * in, size_t const inlen, uint32_t* out_hist,
  ...
  template <typename E>
  int SEQ_histogram_Cauchy_v2(E * in, size_t const inlen, uint32_t* out_hist,
...
#define INIT_HISTSP_SEQ(E)                                                   \
  template int fz::module::SEQ_histogram_Cauchy_v2<E>(                      \
      E * in, size_t const inlen, uint32_t* out_hist, uint16_t const outlen, \
      float* milliseconds);

INIT_HISTSP_SEQ(uint8_t)
INIT_HISTSP_SEQ(uint16_t)
INIT_HISTSP_SEQ(uint32_t)
INIT_HISTSP_SEQ(float)
