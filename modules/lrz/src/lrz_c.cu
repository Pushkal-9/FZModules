...
template <typename T, int TileDim, int Seq, bool UseZigZag,
          typename Eq = uint16_t, typename CompactVal = T,
...
template <typename T, bool UseZigZag, typename Eq = uint16_t,
...
template <typename T, bool UseZigZag, typename Eq = uint32_t, typename Fp = T,
          typename CompactVal = T, typename CompactIdx = uint32_t,
...
template <typename TIN, typename TOUT, bool ReverseProcess = false,
          typename Fp = TIN, int TileDim = 256, int Seq = 8>
__global__ void KERNEL_CUHIP_lorenzo_prequant(TIN* const in,
                                              size_t const in_len,
                                              Fp const ebx2_r, Fp const ebx2,
                                              TOUT* const out) {
  constexpr auto NumThreads = TileDim / Seq;
  auto id_base = blockIdx.x * TileDim;
  ...
  if (d == 1)
    fz::KERNEL_CUHIP_c_lorenzo_1d1l<
        ...
        <<<c_lorenzo<1>::thread_grid(data_len3), c_lorenzo<1>::thread_block, 0,
           (GPU_BACKEND_SPECIFIC_STREAM)stream>>>(in_data, data_len3, leap3,
                                                  out_eq, ot->val(), ot->idx(),
                                                  ot->num(), radius, (T)ebx2_r);
  else if (d == 2)
    fz::KERNEL_CUHIP_c_lorenzo_2d1l<T, UseZigZag, Eq>
    ...
    return FZMOD_NOT_IMPLEMENTED;

  return FZMOD_SUCCESS;
}

template <typename TIN, typename TOUT, bool ReverseProcess>
fzmoderror GPU_lorenzo_prequant(TIN* const in, size_t const len,
  ...
  fz::KERNEL_CUHIP_lorenzo_prequant<TIN, TOUT, ReverseProcess, TIN,
                                     c_lorenzo<1>::tile.x,
