...
template <typename T, int TileDim, int Seq, bool UseZigZag,
          typename Eq = uint16_t, typename Fp = T>
...
#pragma unroll
  ...
  auto block_scan_1d = [&]() {
    ...
    fz::SUBR_CUHIP_WAVE32_intrablock_exclscan_1d<T, Seq, NTHREAD>(
...
#pragma unroll
    for (auto i = 0; i < YSEQ; i++) {
      for (auto d = 1; d < TileDim; d *= 2) {
        T n = __shfl_up_sync(0xffffffff, thp_data[i], d,
  ...
  if (d == 1)
    fz::KERNEL_CUHIP_x_lorenzo_1d1l<
        ...
        <<<x_lorenzo<1>::thread_grid(data_len3), x_lorenzo<1>::thread_block, 0,
           (GPU_BACKEND_SPECIFIC_STREAM)stream>>>(in_eq, in_outlier, out_data,
                                                  data_len3, data_leap3, radius,
                                                  (T)ebx2);
  else if (d == 2)
    fz::KERNEL_CUHIP_x_lorenzo_2d1l<T, UseZigZag, Eq>
...
#define INSTANTIATE_GPU_L23X_3params(T, USE_ZIGZAG, Eq)               \
  template fzmoderror fz::module::GPU_x_lorenzo_nd<T, USE_ZIGZAG, Eq>( \
      Eq* const in_eq, T* const in_outlier, T* const out_data,        \
      stdlen3 const data_len3, f8 const eb, uint16_t const radius,    \
      void* stream);
