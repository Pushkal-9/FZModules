...
struct psz::FzgCodec::impl {
 public:
  ...
  void encode(E* in_data, size_t const data_len, uint8_t** out_archive,
              size_t* archive_len, void* stream) {
    fzgpu::cuhip::GPU_FZ_encode(in_data, data_len, buf->d_offset_counter,
                                buf->d_bitflag_array, buf->d_start_pos,
                                buf->d_comp_out, buf->d_comp_len,
                                (cudaStream_t)stream);

    cudaStreamSynchronize((cudaStream_t)stream);

    cudaMemcpy(&(buf->h_offset_sum), buf->d_offset_counter, sizeof(uint32_t),
               cudaMemcpyDeviceToHost);

    // output
    make_metadata();
    cudaMemcpy(buf->d_archive, &header, sizeof(fzg_header),
               cudaMemcpyHostToDevice);
    *out_archive = buf->d_archive;
    *archive_len = (size_t)header.entry[FZGHEADER_END];
  }

  void decode(uint8_t* in_archive, size_t const archive_len, E* out_data,
...
#define FZG_ACCESSOR(SYM, TYPE) \
  reinterpret_cast<TYPE*>(in_archive + header.entry[FZGHEADER_##SYM])
    ...
    //     (cudaStream_t)stream);
    fzgpu::cuhip::GPU_FZ_decode(FZG_ACCESSOR(BITSTREAM, uint8_t),
                                FZG_ACCESSOR(BITFLAG, uint32_t),
                                FZG_ACCESSOR(START_POS, uint32_t), out_data,
                                data_len, (cudaStream_t)stream);
