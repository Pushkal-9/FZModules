...
int GPU_FZ_encode(uint16_t const* in_data, size_t const data_len,
                  uint32_t* space_offset_counter, uint32_t* out_bitflag_array,
                  uint32_t* out_start_pos, uint8_t* out_comp,
                  uint32_t* comp_len, cudaStream_t stream);

int GPU_FZ_decode(uint8_t const* in_archive, uint32_t* in_bitflag_array,
                  uint32_t* in_start_pos, uint16_t* out_decoded,
                  size_t const decoded_len, cudaStream_t stream);
}  // namespace fzgpu::cuhip
