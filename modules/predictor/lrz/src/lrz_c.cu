...
int GPU_lorenzo_prequant(
    TIN* const in, size_t const len, double const ebx2, double const ebx2_r, TOUT* const out, void* stream)
{
  using namespace fz::kernelconfig;

  fz::KERNEL_CUHIP_lorenzo_prequant<
...
#define INSTANCIATE_GPU_L23_PREQ_3params(TIN, TOUT, REV)                                \
  template int fz::module::GPU_lorenzo_prequant<TIN, TOUT, REV>(                       \
...
INSTANCIATE_GPU_L23R_1param(float);
INSTANCIATE_GPU_L23R_1param(double);

#undef INSTANCIATE_GPU_L23R_1param
#undef INSTANCIATE_GPU_L23R_2params
#undef INSTANCIATE_GPU_L23R_3params

INSTANCIATE_GPU_L23_PREQ_1param(float);
INSTANCIATE_GPU_L23_PREQ_1param(double);
