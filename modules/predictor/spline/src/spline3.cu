...
#define INSTANTIATE_PSZCXX_MODULE_SPLINE__2params(T, E)                                       \
  ...
  template int fz::module::GPU_reverse_predict_spline<T, E>(                                 \
      ...
      T* out_xdata, stdlen3 const xdata_len3, double const ebx2, double const eb_r, uint32_t radius,  \
      void* stream);

#define INSTANTIATE_PSZCXX_MODULE_SPLINE__1param(T) \
  INSTANTIATE_PSZCXX_MODULE_SPLINE__2params(T, uint8_t); \
  INSTANTIATE_PSZCXX_MODULE_SPLINE__2params(T, uint16_t);

INSTANTIATE_PSZCXX_MODULE_SPLINE__1param(float);
INSTANTIATE_PSZCXX_MODULE_SPLINE__1param(double);
