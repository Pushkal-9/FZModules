...
void fzgpu::Buf::init(size_t data_len, bool _alloc_test_buf)
{
  ...
  max_archive_bytes = sizeof(fzg_header)                            // field 1: header
...
{
  cudaMemset(d_archive, 0, max_archive_bytes);
  cudaMemset(d_offset_counter, 0, sizeof(uint32_t));
  cudaMemset(d_comp_len, 0, sizeof(uint32_t) * config.at("grid_x"));
};
