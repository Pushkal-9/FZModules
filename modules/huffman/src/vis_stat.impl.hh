...
double get_entropy(T* code, size_t l, size_t cap) {
  ...
  std::copy_if(raw.begin(), raw.end(), std::back_inserter(frequencies),
               [](double& e) { return e != 0; });
  double entropy = 0;
...
template <typename T>
void visualize_histogram(const std::string& tag, T* _d_POD, size_t l,
                         size_t _bins, bool log_freq, double override_min,
                         double override_max, bool eliminate_zeros,
  ...
  if (eliminate_zeros) {
    std::copy_if(_d.begin(), _d.end(), std::back_inserter(_d_nonzero),
                 [](int i) { return i != 0; });
  }
  ...
  if (override_max > override_min) {
    ...
    std::tie(max_v, min_v, rng) = std::make_tuple(override_max, override_min,
